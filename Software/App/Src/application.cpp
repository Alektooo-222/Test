...
extern uint32_t LengthRxMess_USB;
...
etl::queue_spsc_atomic<StringTxQueue, SIZE_TX_QUEUE> tx_queue;
etl::queue_spsc_atomic<StringTxQueue, SIZE_TX_QUEUE> tx_usb_queue;
...
void CommandManager()
{
    etl::string<SIZE_RX_MESS> s(reinterpret_cast<char *>(UserRxBuffer), reinterpret_cast<char *>(UserRxBuffer) + static_cast<size_t>(LengthRxMess_USB));
...
void cmd_echo_handler(string_rx_mess &str)
{
    ...
    /* while (!tx_usb_queue.push(StringTxQueue(str)))
        ; */
    push_with_timeout(tx_queue, str, 100);
}
...
void cmd_pinout_handler(string_rx_mess &str)
{
    ...
    add_str(string_periph, str);
    add_str(string_periph, ":\n");

    for (auto &pin : ListPins)
    {
        add_str(string_periph, pin.name.pin_name);
        add_str(string_periph, "\n");
        add_str(string_periph, staus_table[pin.conf]);
        add_str(string_periph, "\nPeriph: ");
        ...
        else
        {
            ...
            for (auto &tim : pin.hardware_features)
            {
                if (i != 0)
                    add_str(string_periph, "        ");
                add_str(string_periph, tim_table[tim.timer]);
                add_str(string_periph, "_");
                add_str(string_periph, ch_table[tim.channel]);
                add_str(string_periph, "\n");
                i++;
            }
        }
        ...
        if (!push_with_timeout(tx_queue, string_periph, 100))
        {
        }
