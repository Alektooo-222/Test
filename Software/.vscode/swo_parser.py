...
if sys.platform == "win32":
    os.system('')  # Включить ANSI-поддержку на Windows
...
class Stream:
    ...
    def __init__(self, id, header = '', tcl_socket = None, color=None):
        self.id = id;
        self._buffer = []
        self._header = header
        self.tcl_socket = tcl_socket
        self.color = color  # Цвет для этого потока
    ...
    def _output(self, s):
        if self.color:
            print(f"{self.color}{s}\033[0m")  # Цветной вывод + сброс
        else:
            print(s)
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as tcl_socket:
    ...
    streams.add_stream(Stream(0, '', tcl_socket, color='\033[32m'))
    streams.add_stream(Stream(1, 'WARNING: ', None, color='\033[33m'))
    streams.add_stream(Stream(2, 'ERROR: ', tcl_socket, color='\033[31m'))
